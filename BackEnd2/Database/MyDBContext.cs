...
namespace BackEnd2.Database
{
    public class MyDBContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Produit>().ToTable(nameof(Product)).HasOne<Reed>(pr=>pr.PeigneObj).WithMany(pn=>pn.Products).HasForeignKey(pr=>pr.Peigne);
            modelBuilder.Entity<Produit>().ToTable(nameof(Product)).HasOne<Redacteur>(pr=>pr.RedacteurObj).WithMany(red=>red.Products).HasForeignKey(pr=>pr.Redacteur);
            ...
            modelBuilder.Entity<Produit>().HasMany(pr => pr.GetComposition).WithOne(comp => comp.ProdID).OnDelete(DeleteBehavior.Cascade);
        ...
        public List<FicheTechnique> GetFicheTechniques()
        {
            return FicheTec.Include(ft=>ft.Catalog)
                ...
                .Include(ft=>ft.Produits).ThenInclude(pr=>pr.GetComposition).ThenInclude(comp=>comp.GetMatiere).ThenInclude(ma=>ma.Titrage).ThenInclude(tit=>tit.TypeMatiere)
                .Include(ft=>ft.Produits).ThenInclude(pr=>pr.GetComposition).ThenInclude(comp=>comp.GetMatiere).ThenInclude(ma =>ma.GetCouleur)
                .Include(ft=>ft.Produits).ThenInclude(pr=>pr.GetComposition).ThenInclude(compo=>compo.GetComposant)               
                .Include(ft=>ft.Produits).ThenInclude(pr=>pr.EnfilageID).ThenInclude(enf=>enf.GetMatrix)
                .Include(ft=>ft.Produits).ThenInclude(pr=>pr.EnfilageID).ThenInclude(enf=>enf.GetChaine).ThenInclude(ch=>ch.ChMatrix)
        ...
        {
            return Composition.Where(compo=>compo.ProdID.Id==ProdId && compo.ProdID.Version==ProdVersion).ToList();
        }
        ...
        { 
          Produit prod=  Produit.ToList().Find(pr => pr.NumArticle == NumArticle && pr.Id!=ProdID);
          if (prod != null)
          {
              return false;
          }
        ...
        { 
            Produit prod=  Produit.ToList().Find(pr => pr.Id == NewProd.Id & pr.Version==NewProd.Version);
            if (prod != null)
            {
                return false;
            }
        ...
        public Composition AddProdCompo(Composition compo)
        {
            ...
            this.Entry(compo).State = EntityState.Added;
            if (compo.ProdID != null)
            {
                Produit prod= Produit.SingleOrDefault(pr => pr.Id == compo.ProdID.Id);
                compo.ProdID = prod;
            }
        ...
        public void RemoveProdCompo(Composition compo)
        {
           ...
           Composition delComp= Composition.SingleOrDefault(cmp => cmp.ID == compo.ID);
           if (delComp != null)
           {
        ...
        public void RemoveEnfilage(Enfilage mEnfilage)
        {
            ...
            Enfilage DelEnf = Enfilage.SingleOrDefault(enf => enf.ID == mEnfilage.ID);
            if(DelEnf!=null)
              {
        ...
        public void RemoveEnfilageElement(EnfilageMatrix EnfilageElement)
        {
            ...
            EnfilageMatrix enfmat = EnfilageMatrix.SingleOrDefault(enfmx => enfmx.ID == EnfilageElement.ID);
            if (enfmat != null)
            {
        ...
        public void RemoveChaineElement(ChaineMatrix ChaineElement)
        {
            ...
            ChaineMatrix chmat = ChaineMatrix.SingleOrDefault(chmatx => chmatx.ID == ChaineElement.ID);
            if (chmat != null)
            {
        ...
        {
            return Matiere.Include(ma => ma.Titrage).Include(tits => tits.GetCouleur).ToList().FindAll(ma => ma.Titrage.ID==tit.ID).Select(ma => ma.GetCouleur).ToList();
        }
        ...
        {
            Machine mch = mMachine.SingleOrDefault(machin => machin.ID == mm.ID);
            if (mch != null)
            {
                mMachine.Remove(mm);
                SaveChanges();
        ...
        public void DeleteFicheTechnique(FicheTechnique fiche)
        {
            ...
            List<int> EnfilageIDs = new List<int>();
            ...
            var DelFiche= FicheTec.Include(ft=>ft.Produits).ThenInclude(pr=>pr.GetComposition).SingleOrDefault(ft => ft.ID == fiche.ID);

            if (DelFiche.Produits.Count > 0)
            {
                foreach (var prod in DelFiche.Produits)
                {
                    List<Composition> TempoList =new List<Composition>( prod.GetComposition);
            ...
            foreach (var DelProd in DelFiche.Produits)
            {
               
                
                Produit.Remove(DelProd);
        ...
        public void DeleteFicheTechnique(int ficheID,int prodID)
        {
           ...
           var DelProd = Produit.Include(pr=>pr.EnfilageID).SingleOrDefault(pr => pr.Id == prodID);
            if (DelProd!=null)
            {
                ...
                Enfilage.Remove(DelProd.EnfilageID);
            }
        ...
        {
           EnfilageMatrix DelEl= EnfilageMatrix.SingleOrDefault(enf => enf.EnfID == EnfID && enf.x==x && enf.y==y);
        ...
        {
            EnfilageMatrix UpdateEL= EnfilageMatrix.SingleOrDefault(enf => enf.EnfID == EnfID && enf.x==x && enf.y==y);
        ...
        {
            Enfilage UpdateEnfilage= Enfilage.SingleOrDefault(enf => enf.ID == Menfilage.ID );
