        ...
        public void GetUpperDataSheetPart(Produit pr, SQLiteConnection conn)
        {
           var stm1 = "Select * from (select * from Product where Id=@id) as pr" +
                ...
                }, obj =>
                {
                    ...
                    pr.Version = prd.Version;
                    pr.DateCreation = prd.DateCreation;
                    pr.MiseAJour = prd.MiseAJour;
                    pr.Name = prd.Name;
                    pr.Name2 = prd.Name2;
                    pr.NumArticle = prd.NumArticle;
                    pr.Ref = prd.Ref;
                    pr.Largeur = prd.Largeur;
                    pr.Epaisseur = prd.Epaisseur;
                    pr.image = prd.image;
                    pr.Dent = prd.Dent;
                    pr.EnfDent = prd.EnfDent;
                    pr.Client = cl;
                    pr.DuitageID = duit;
